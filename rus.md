...
которые доступны нам на сегодняшний день. Контент обычно
растягивает контейнер по вертикали. Если по условиям макета элементы должны
сохранять определённую высоту, выбор решений ограничен: можно
либо добавить полосу прокрутки, либо спрятать контент, который не поместился. Спецификация CSS-областей (CSS Regions) предлагает совершенно новый вариант.

## Поддержка

Области являются новой частью спецификации CSS, поэтому на данный момент они поддерживаются не везде, в некоторых браузерах они находятся за флагом.
Поддержка областей недавно была добавлена в iOS7, Safari 7, также они поддерживаются Safari 6.1+.
Команда Adobe составила [список браузеров, поддерживающих разные новые возможности (включая области), и инструкции по
их активации][1]. Как бы то ни было, поддержка
областей постоянно растёт. Полный список браузеров, которые поддерживают области
и другие продвинутые возможности, можно увидеть на сайта Adobe на [странице «Поддержка
CSS-областей»][2].
...
которая является группой элементов, между которыми распределяется поток.
...
`flow-from` с тем же названием потока в качестве значения. Содержимое первого элемента
будет перетекать в остальные элементы-области. На данный момент браузеры
требуют свойства с префиксом, но здесь мы используем версию без
префикса:
...
в элементах-областях. Нужно прописать этот псевдоселектор в качестве селектора области, а затем
передать селектор элемента в качестве аргумента, обозначая для каких элементов будет
...
`height: auto`, чтобы быть уверенным что она расширится, чтобы вместить
...
Области определяются исключительно в CSS, что делает очень простым их использование в
сочетании с медиазапросами. Кроме изменения размеров и
...
Благодаря этой возможности отдельные области можно легко удалить из страницы не
...
Единицы области просмотра позволяют использовать окно (или область просмотра) как
...
Области открывают множество возможностей для разработки  отзывчивого дизайна и обеспечения
превосходного представления контента независимо от размеров области просмотра. Один
...
следить за последними обновлениями по областям и другим новинкам.
...
Полную информацию можно найти в Спецификации для CSS-областей, в которой
описаны все вопросы, рассмотренные в этой статье и многие другие. Также дополнительную
информацию можно найти в разделе «[Области][14]» на сайте Adobe.
